...
def visualize_vedo_vectorarray(grid, U, bounding_box=([0, 0], [1, 1]), codim=2, title=None, legend=None,
                    separate_colorbars=False, rescale_colorbars=False, columns=2, color_map=get_cmap('viridis')):
    """Generate a pyvedo Plot and associated controls for scalar data associated to a
    ...
    return visualize_vedo(filenames, bounding_box, codim, title, legend, separate_colorbars, rescale_colorbars,
                           columns, color_map)


def visualize_vedo(vtk_filelikes, bounding_box=([0, 0], [1, 1]), codim=2, title=None, legend=None,
                   separate_colorbars=False, rescale_colorbars=False, columns=2, color_map=get_cmap('viridis')):
    import vedo
    mesh = vedo.read(vtk_filelikes[0])
    cpos = mesh.plot()
    return cpos
